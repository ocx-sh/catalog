...
describe('usePackageRoot', () => {
  ...
  test('a nav race during the canonical fallback also discards the stale write', async () => {
    // The fourth staleness point, and the one the alias fallback added: the
    // alias 404s, so the CANONICAL fetch is the slow step, and the nav
    // happens while it is in flight. Its token guard sits between that
    // second fetch and the 404 check below it — without it, an older
    // package's canonical root lands on a page already showing a newer one.
    const canonical = new Map<string, (v: unknown) => void>()
    globalThis.fetch = vi.fn((url: string) =>
      url.endsWith('/_root.json')
        ? Promise.resolve({ ok: false, status: 404 })
        : new Promise((resolve) => canonical.set(url, resolve)),
    ) as unknown as typeof fetch

    const { get, wrapper, nsRef, pkgRef } = mountHost('kitware', 'cmake')
    await vi.waitFor(() => expect(canonical.has('/p/kitware/cmake.json')).toBe(true))

    nsRef.value = 'ocx-contrib'
    pkgRef.value = 'shellcheck'
    await vi.waitFor(() => expect(canonical.has('/p/ocx-contrib/shellcheck.json')).toBe(true))

    const ok = (name: string) => ({ ok: true, status: 200, json: () => Promise.resolve({ name }) })
    canonical.get('/p/ocx-contrib/shellcheck.json')!(ok('fresh'))
    canonical.get('/p/kitware/cmake.json')!(ok('stale'))
    await vi.waitFor(() => expect(get().loading.value).toBe(false))
